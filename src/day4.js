export const parseRecord = record => {
  ...
  return [new Date(timestamp.trim().slice(1)), message.trim()];
};

const parseRecordEntries = (recordEntries, delimiter = '\n') =>
  recordEntries.split(delimiter).map(parseRecord);

export const getOrderedRecordEntries = (recordEntries, delimiter = '\n') =>
  parseRecordEntries(recordEntries, delimiter).sort(
    ([firstTimestamp, firstMessage], [secondTimeStamp, secondMessage]) =>
      firstTimestamp > secondTimeStamp ? 1 : -1
  );

const MESSAGES = {
  SHIFT_START: 'begins shift',
  SLEEPS: 'falls asleep',
  WAKES: 'wakes up',
};

const mapSleepingRecords = (recordEntries, delimiter = '\n') =>
  getOrderedRecordEntries(recordEntries, delimiter).reduce(
    (prev, [timestamp, message], index, recordsArray) => {
      const currentDateString = `${timestamp.getMonth() +
        1}-${timestamp.getDate()}`;
      ...
      if (message.includes(MESSAGES.SLEEPS)) {
        ...
        return {
          ...
          [currentDateString]: {
            id,
            sleepTimeArray: sleepTimeArray.map(
              (value, index) =>
                value || (index >= firstSleepMinute && index < wakeUpMinute)
            ),
          },
        };
      }
      if (message.includes(MESSAGES.WAKES)) {
        return prev;
      }
    },
    {}
...
export const guardTimesMostAsleepMinute = (recordEntries, delimiter = '\n') => {
  ...
  const laziestGuardAndMinutes = Object.entries(guardToSleepingMinutes).reduce(
    ([prevId, prevSleepArray], [currId, currSleepArray]) => {
      ...
      const prevAmounts = prevSleepArray.reduce((prev, curr) => prev + curr, 0);
      return currAmounts > prevAmounts
